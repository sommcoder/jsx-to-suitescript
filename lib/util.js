...
export const constructId = (label, el, parent, container) => {
    // label is the only thing that the user would manually input. The other inputs are inferred by the structure of the JSX.
    if (!label) {
        throw new Error(`label is required and was undefined, please provide a string as the value for the 'label' property on the elemetnt: ${el}, located in parent: ${parent} `);
    }
    if (label.match(/^[a-zA-Z0-9]*$/)) {
        const labelStr = label.split(" ").join("_");
        const id = `custpage_${labelStr}_${el}${container ? "_" + container : "_" + parent}`.toLowerCase();
        return id;
    }
    else
        throw new Error(`ERROR: The element ${el} contains non-alphanumeric characters in its label proprty: ${label}`);
};
